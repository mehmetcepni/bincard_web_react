...
const statusLabels = (t) => ({
  ...
  COMPLETED: t('wallet.statusCompleted'),
...
const TransferDetail = ({ transferId }) => {
  ...
  useEffect(() => {
    ...
    walletService.getTransferDetail(transferId)
      .then(setTransfer)
      .catch(err => setError(err.message || t('wallet.transferError')))
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[200px]">
        ...
        <span className="text-blue-700 font-medium">{t('wallet.transferLoading')}</span>
      </div>
    );
  }
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 text-red-700 rounded-lg p-6 text-center">
        <div className="mb-2 font-bold text-lg">{t('wallet.error')}</div>
        <div>{error}</div>
      </div>
    );
  }
    ...
    <div className="max-w-md mx-auto bg-white rounded-2xl shadow-2xl p-8 border border-gray-100 mt-8">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-bold text-blue-700">{t('wallet.transferDetail')}</span>
        </div>
      ...
      <div className="flex flex-col items-center mb-6">
        ...
        <div className="text-gray-500 text-sm">{t('wallet.transferId')}: <span className="font-mono">{transfer.id}</span></div>
      </div>
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-700 font-medium">{t('wallet.initiatedAt')}:</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-700 font-medium">{t('wallet.status')}:</span>
          <span className="ml-auto text-gray-900">{statusLabels(t)[transfer.status] || transfer.status}</span>
        </div>
      </div>
    </div>
  );
